...
// S3 USB-webconfig (Task 7): SwitchPro function byte-identical to
// switch_pro_configuration_descriptor, followed by the RNDIS function
// (TUD_RNDIS_DESCRIPTOR argument order: itf, str, ep_notif, notif_size,
// epout, epin, epsize). The SwitchPro function uses IN 0x81 / OUT 0x01, so
// the first-free rule gives notif intr 0x83, bulk OUT 0x04, bulk IN 0x85;
// RNDIS takes interfaces 1+2, hence bNumInterfaces 1 -> 3 and wTotalLength
...
static const uint8_t switch_pro_configuration_descriptor_with_net[] =
{
    0x09,        // bLength
    0x02,        // bDescriptorType (Configuration)
